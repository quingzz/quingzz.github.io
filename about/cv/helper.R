...
# function to generte project list
render_prj_list <- function(items){
  # Render each project as formatted text
  items %>%
    mutate(
      output = pmap_chr(
        list(name = name, type = type, role = role, 
             url_github = url_github, url = url, description = description),
        \(name, type, role, url_github, url, description) {
          github_link <- if (!is.na(url_github)) glue("\\href{{{url_github}}}{{GitHub}}") else NULL
          website_link <- if (!is.na(url)) glue("\\href{{{url}}}{{Website}}") else NULL
          
          links <- c(github_link, website_link) %>% discard(is.null) %>% 
            str_c(collapse = " | ")
          
          glue("\\textbf{{{name}:}} {description}\n\n\\textit{{Role: {role}{if (links != '') glue(' | {links}') else ''}}}")
        }
      )
    ) %>%
    pull(output) %>%
