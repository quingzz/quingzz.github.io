...
description: "Overview of methods for Bayesian inference, and its potential application in deep probabilistic programming"
author: Anh Phan
date: "2026-04-24"
...
That is to say, we need integrate over [all the possible values]{.underline} in the parameter space ( $\Theta$ ). In most realistic models, it is impossible to derive this integral in closed form (with a caveat, see @note-conjugate-prior) or requires exponential time to compute.
...
```{r}
#| code-fold: true
#| code-summary: "Functions to compute log priors"
...
```{r}
#| code-fold: true
#| code-summary: "Functions to compute the expected values"
...
```{r}
#| code-fold: true
#| code-summary: "Function to implement CAVI"
...
```{r}
#| code-fold: true
#| code-summary: "Code for plotting ELBOs"
cavi_out <- cavi(y)
