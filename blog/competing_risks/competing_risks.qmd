...
A generalized formulation for $\frac{dI_{i,j}}{dt}$, $\frac{dR}{dt}$, $\frac{dD}{dt}$ is as followed
...
Implementation for the scenario where $T_{I \rightarrow R}$ and $T_{I \rightarrow D}$ are Erlang distributed in odin
...
sird_lct <- odin({
  # ODE in odin syntax
  deriv(S) <- - beta * ((I11 + I12 + I21 + I22)/N) * S
  deriv(I11) <- beta * ((I11 + I12 + I21 + I22)/N) * S - (gamma + mu)*I11
  deriv(I12) <- mu*I11 - (gamma + mu)*I12
  deriv(I21) <- gamma*I11 - (gamma + mu)*I21
  deriv(I22) <- gamma*I12 + mu*I21 - (gamma + mu)*I22
  deriv(R) <- gamma*I21 + gamma*I22
  deriv(D) <- mu*I12 + mu*I22
...
There have been several methods proposed to handle arbitrary dwell time distribution in compartmental model, using delay-integro differential equations or delay differential equation (DDE).

This blog, however, aim to re-frame the problem under the context of survival analysis to handle compartmental model with independent, arbitrarily distributed competing risks.

### Competing risks in terms of survival function

Consider the scenario where compartment $X$ can transition to $n$ out compartments denoted $O_i$ where $i \in \{1,2,...,n\}$. We model these out transition as $n$ competing risks (assumed to be independent) with arbitrary parametric distribution.

Some annotations and indications within the context of compartmental model:

-   $f(t)$ is the probability distribution function (PDF) - indicate the instantaneous probability that an individual leave the compartment at time $t$.
...
-   The subset of function annotation denotes which event this function is for. For example, $f_{X \rightarrow O_1}(t)$ refers to the PDF for the $X \rightarrow O_1$ transition.
...
## Why the overall survival function looks like that?
...
$$
h_X(t) = \frac{-\frac{d}{dt} S_X(t)}{S_X(t)} = \frac{-\frac{d}{dt} \prod_{i = 1}^{n} S_{X \rightarrow O_i}(t)}{\prod_{i = 1}^{n} S_{X \rightarrow O_i}(t)}
$$

After a few steps of simplifying the function, we will eventually arrive at the following.

$$
h_X(t) = \sum_{i = 1}^n\frac{ f_{X \rightarrow O_i}(t)}{S_{X \rightarrow O_i}(t)}
$$
...
The takeaway point here is that the hazard function of leaving $X$ compartment can be rewritten in terms of the hazard function of each competing risk $X \rightarrow O_i$.
...
$$
\begin{cases}
\frac{dX(t)}{dt} = - \int_{0}^{\tau} h_X(\tau)*X(t, \tau) d\tau \\
\frac{dO_1(t)}{dt} = \int_{0}^{\tau} h_{X\rightarrow O_1}(\tau)*X(t,\tau)d\tau \\
\vdots \\
\frac{dO_n(t)}{dt} = \int_{0}^{\tau} h_{X\rightarrow O_n}(\tau)*X(t,\tau)d\tau \\
\end{cases}
$$
...
$$
\frac{dI(t)}{dt} = - \int_0^\tau (\gamma + \mu) I(t,\tau)d \tau = - (\gamma + \mu) \int_0^\tau I(t,\tau)d \tau 
$$\
...
mod_config <- list(
  r_dist = pgamma,
  r_params = list(rate=0.2, shape=2),
  d_dist = pgamma,
