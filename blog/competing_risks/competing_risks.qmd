...
-   The subscript of function annotation denotes which event this function is for. For example, $f_{X \rightarrow O_1}(t)$ refers to the PDF for the $X \rightarrow O_1$ transition.
...
<!-- -->
```
-   $T_{I\rightarrow R} \sim \text{Erlang}(\text{rate = }\gamma, \text{shape = }2)$
...
Recall @eq-comp-hazard, we need to keep track of $I(t, \tau)$ which is the sub-population of $I(t)$ that have stayed for $\tau$ time period. In discrete time, it is equivalent to dividing $I(t)$ into $\frac{\tau}{timestep}$ sub-compartments.
...
```{r}
# TODO: include model implemented in denim
```
...
-   For implementing Erlang distributed $T_{I \rightarrow R}$ and/or $T_{I \rightarrow D}$ using a system of ODE.
